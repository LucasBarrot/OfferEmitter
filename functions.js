...
const truckOnTheRoad = async (Truck, offer, dataBase) => {
    ...
    isArrived.then((value) => {
        ...
        Truck.status = value;
        console.log(Truck.name + ": Shipping done. Cost :" + Math.ceil(Truck.consumption * offer.travel * 0.01));
        return dataBase
    });
};
...
const analyzedOffer = (offer, dataBase) => {
    ...
    console.log("New offer :\n" + "Boxes :" + offer.boxes + "\nPrice per box :" + offer.pricePerBox + "\nTravel :" + offer.travel + "\n");
    if (offer.pay > (offer.boxes / dataBase.Trucks[0].capacity * dataBase.Trucks[0].consumption * offer.travel * 0.01)) {
